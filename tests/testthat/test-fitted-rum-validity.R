...
test_that("fitted RUM eligibility rejects invalid signs, bounds, sizes, and normalization", {
  source <- suppressWarnings(specify(
    "logit", "bertrand", prices = c(2, 2.2, 2.5),
    parameters = list(alpha = -1.5, meanval = c(1, .8, 1.2)),
    ownerPre = c("A", "B", "C"), insideSize = 100
  ))
  target <- .fitted_rum_validity_wrapper(source, "LogitALM")
  expect_true(.has_fitted_rum_state(target))

  bad_alpha <- target
  bad_alpha@slopes$alpha <- abs(bad_alpha@slopes$alpha)
  expect_false(.has_fitted_rum_state(bad_alpha))

  bad_inside_size <- target
  bad_inside_size@insideSize <- 0
  expect_false(.has_fitted_rum_state(bad_inside_size))

  bad_market_size <- target
  bad_market_size@mktSize <- NA_real_
  expect_false(.has_fitted_rum_state(bad_market_size))

  bad_shares <- target
  bad_shares@shares[1] <- 0
  expect_false(.has_fitted_rum_state(bad_shares))

  bad_norm <- target
  bad_norm@normIndex <- 0L
  expect_false(.has_fitted_rum_state(bad_norm))

  no_outside <- target
  no_outside@shares <- c(.3, .3, .4)
  no_outside@shareInside <- 1
  no_outside@normIndex <- 1L
  expect_true(.has_fitted_rum_state(no_outside))
  no_outside@normIndex <- NA_integer_
  expect_false(.has_fitted_rum_state(no_outside))

  bad_ces_output <- suppressWarnings(specify(
    "ces", "bertrand", prices = c(2, 2.2, 2.5),
    parameters = list(gamma = 3, alpha = .4, meanval = c(1, .8, 1.2)),
    ownerPre = c("A", "B", "C"), insideSize = 100
  ))
  bad_ces_output <- .fitted_rum_validity_wrapper(bad_ces_output, "CESALM")
  bad_ces_output@slopes$gamma <- 1
  expect_false(.has_fitted_rum_state(bad_ces_output))

  ces_input <- suppressWarnings(specify(
    "ces", "bertrand", prices = c(2, 2.2, 2.5),
    parameters = list(gamma = -1, alpha = .4, meanval = c(1, .8, 1.2)),
    ownerPre = c("A", "B", "C"), insideSize = 100,
    output = FALSE
  ))
  ces_input <- .fitted_rum_validity_wrapper(ces_input, "CESALM")
  expect_true(.has_fitted_rum_state(ces_input))
  ces_input@slopes$gamma <- 0
  expect_false(.has_fitted_rum_state(ces_input))
})
