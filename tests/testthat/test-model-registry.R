test_that("the registry covers every currently supported sim combination", {
    ...
    expected <- data.frame(
        ...
        variant = c(rep("standard", 18), rep("alm", 10)),
        class = c(
            ...
            "LogitBLP", "CournotBLP", "LogitALM", "LogitCournotALM",
        ...
        calibrator = c(
            ...
            "sim", "sim", "logit.alm", "logit.cournot.alm", "ces.alm",
        ...
        calibrate = c(rep(TRUE, 16), FALSE, FALSE, rep(TRUE, 10)),
        specify = c(rep(TRUE, 18), rep(FALSE, 10)),
        simulate = rep(TRUE, 28),
