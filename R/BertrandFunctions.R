...
bertrand.alm <- function(
  ...
  if (demand == "aids") {
    if (missing(prices)) {
      prices <- rep(NA_real_, length(shares_revenue))
    }
    ...
    if (missing(parmStart)) {
      ...
      if (demand == "logit") {
        ...
        parmStart[1] <- 1 / (margins[nm] * (1 - shares_revenue[nm])) - shares_revenue[nm] / (1 - shares_revenue[nm])
      } # ballpark gamma for starting values
    }
  ...
  result <- switch(demand,
    aids = new("AIDS",
      shares = shares_revenue, mcDelta = mcDelta, subset = subset,
    ...
    logit = new("LogitALM",
      prices = prices, shares = shares_quantity,
      margins = margins,
      ownerPre = ownerPre,
      ownerPost = ownerPost,
      mktElast = mktElast,
    ...
    ces = new("CESALM",
      prices = prices, shares = shares_revenue,
      margins = margins,
      ownerPre = ownerPre,
      ownerPost = ownerPost,
      mktElast = mktElast,
    ...
    linear = new("Linear",
      prices = prices, quantities = shares_quantity, margins = margins,
