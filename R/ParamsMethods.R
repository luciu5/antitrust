...
setMethod(
  ...
  definition = function(object) {
    ...
    if (!noCosts) {
      mcPre <- sapply(1:nplants, function(i) {
        object@mcfunPre[[i]](quantities[i, ])
      })
    }
    ...
    if (output) {
      ...
      lowerB <- c(rep(-Inf, nprods), rep(0, nprods))
    ...
    if (isTRUE(all.equal(bestParms[1:nplants], rep(0, nplants), check.names = FALSE))) {
      warning("Some plant-level cost parameters are close to 0.")
    }
    if (isTRUE(all.equal(bestParms[-(1:nplants)], rep(0, nprods), check.names = FALSE))) {
      warning("Some demand parameters are close to 0.")
    }
...
setMethod(
  ...
  definition = function(object) {
    ...
    if (!noCosts) {
      ...
      dmcPre <- sapply(1:nplants, function(i) {
        object@dmcfunPre[[i]](quantities[i, ])
      })
    }
    ...
    if (noCosts) {
      if (isLinearD) {
        margStart <- rowMeans(-(sharesOwner * quantTot) / (prices / bStart), na.rm = TRUE)
      } else {
        margStart <- rowMeans(-sharesOwner * bStart, na.rm = TRUE)
      }
    ...
    if (noCosts) {
      ...
      dmcdef <- lapply(dmcdef, function(x) {
        eval(parse(text = x))
      })
...
setMethod(
  ...
  definition = function(object) {
    ...
    if (!symmetry) {
      ...
      bestParms <- constrOptim(parmStart, minD,
    ...
    if (any(intercept < 0)) {
      warning("Some demand intercepts are negative")
    }
    if (any(diag(slopes) > 0)) {
      warning("Some own-slope coefficients are positive")
    }
...
setMethod(
  ...
  definition = function(object) {
    ...
    cancalibrate <- apply(diversion, 1, function(x) {
      !any(x == 0)
    })
    ...
    bestParms <- constrOptim(parmStart,
      f = minD, ui = ui, ci = ci, grad = NULL,
      control = object@control.slopes
    )
    ...
    if (abs(bestParms$par[2]) > 5) {
      warning("'mktElast' estimate is large: ", bestParms$par[2])
    }
    if (isTRUE(all.equal(bestParms$par[2], object@parmStart[2]))) {
      warning("'mktElast' estimate is not identified")
    }
...
setMethod(
  ...
  definition = function(object) {
    ...
    minNests <- optim(object@nestsParms, minD,
      method = "L-BFGS-B",
      lower = 0, upper = 1,
      control = object@control.slopes
    )$par
...
setMethod(
  ...
  definition = function(object) {
    ...
    minD <- function(theta) {
      ...
      if (any(class(elastInv) == "try-error")) {
        elastInv <- MASS::ginv(elast * ownerPre)
      }
...
setMethod(
  ...
  definition = function(object) {
    ...
    minTheta <- constrOptim(parmsStart, minD,
      grad = NULL, ui = constrA, ci = constrB,
      control = object@control.slopes
    )
...
setMethod(
  ...
  definition = function(object) {
    ...
    minD <- function(theta) {
      ...
      # For Cournot, the margin formula is:
      # margins = output * (1 + sharesFirm / idxShare) / alpha
      # where sharesFirm = owner %*% shares and idxShare is the outside share

      sharesFirm <- as.numeric(ownerPre %*% probs)

      # Predicted margins using Cournot FOC
      # calcMargins returns proportional margins (margin/price), so we need to match that
