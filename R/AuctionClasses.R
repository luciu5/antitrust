...
#'
#' Auction2ndCES: Objects can be created by using the constructor function \code{\link{auction2nd.ces}}.
#'
#' Auction2ndCESALM: Objects can be created by using the constructor function \code{\link{auction2nd.ces.alm}}.
#'
#' @description The \dQuote{Auction2ndCES} class contains all the information needed to
#' calibrate a CES
#' demand system and perform a merger simulation analysis under the assumption that
#' firms are setting offers in a 2nd-score auction.
#' @description The \dQuote{Auction2ndCESALM} class contains all the information needed to
#' calibrate a CES
#' demand system with unobserved outside share and perform a merger simulation analysis under the assumption that
#' firms are setting offers in a 2nd-score auction.
...
setClass(
  ...
  validity = function(object) {
    ...
    if (is.na(object@reserve)) {
      parmsStart <- object@parmsStart[-1]
    } else {
    ...
    if (identical(cdf, "punif")) {
      if (length(parmsStart) != 2) {
        if (is.na(object@reserve)) {
          stop("parmsStart must be a length 3 vector whose first element is the starting value for 'reserve'")
        } else {
          stop("For the Uniform distribution, 'parmsStart' must be a numeric vector of length 2")
        }
      }
      ...
      if (length(parmsStart) != 1) {
        if (is.na(object@reserve)) {
          stop("parmsStart must be a length 2 vector whose first element is the starting value for 'reserve'")
        } else {
          stop("For the Exponential distribution, 'parmsStart' must be a numeric vector of length 1")
        }
      }
      if (parmsStart[1] <= 0) {
        stop(
          "For the Exponential distribution, 'parmsStart' must be a numeric vector of length ",
      ...
      if (length(parmsStart) != 2) {
        if (is.na(object@reserve)) {
          ...
          stop("For the Weibull distribution, 'parmsStart' must be a numeric vector of length 2")
        }
      }
      if (parmsStart[1] <= 0 ||
        parmsStart[2] <= 0) {
        stop(
          "For the Weibull distribution, 'parmsStart' must be a numeric vector of length ",
      ...
      if (length(parmsStart) != 2) {
        if (is.na(object@reserve)) {
          ...
          stop("For the Gumbel distribution, 'parmsStart' must be a numeric vector of length 2")
        }
      }
      if (parmsStart[2] <= 0) {
        stop(
          "For the Gumbel distribution, 'parmsStart' must be a numeric vector of length ",
      ...
      if (length(parmsStart) != 3) {
        if (is.na(object@reserve)) {
          stop("parmsStart must be a length 4 vector whose first element is the starting value for 'reserve'")
        } else {
          stop("For the Frechet distribution, 'parmsStart' must be a numeric vector of length 3")
        }
      }
      if (parmsStart[2] <= 0 ||
        parmsStart[3] < 2) {
        stop(
          "For the Frechet distribution, 'parmsStart' must be a numeric vector of length ",
...
setClass(
  ...
  validity = function(object) {
    ...
    if (!object@constraint &&
      any(
        tapply(
          object@margins[!isSingleton], object@nests[!isSingleton],
          function(x) {
            if (all(is.na(x))) {
              return(TRUE)
            } else {
