...
setMethod(
  ...
  definition = function(object, preMerger = TRUE, level = FALSE) {
    ...
    if (is.na(idx)) {
      idxShare <- 1 - sum(shares, na.rm = TRUE)
    } else {
      idxShare <- shares[idx]
    }
...
setMethod(
  ...
  definition = function(object, preMerger = TRUE, level = FALSE) {
    ...
    if (any(class(elast.inv) == "try-error")) {
      elast.inv <- MASS::ginv(ownerDown * elast)
    }
...
setMethod(
  ...
  definition = function(object, preMerger = TRUE, level = FALSE) {
    ...
    if (preMerger) {
      ...
      if (level) {
        marginPre <- marginPre * object@pricePre
      }
      return(marginPre)
    } else {
...
setMethod(
  ...
  definition = function(object, preMerger = TRUE, exAnte = FALSE, level = TRUE) {
    ...
    if (exAnte) {
      margins[subset] <- margins[subset] * shares
    }
    if (!level) {
      margins <- margins / prices
    }
