...
setGeneric(
  name = "calcQuantities",
  def = function(object, ...) {
    standardGeneric("calcQuantities")
  }
)
setGeneric(
  name = "calcRevenues",
  def = function(object, ...) {
    standardGeneric("calcRevenues")
  }
)
setGeneric(
  name = "calcShares",
  def = function(object, ...) {
    standardGeneric("calcShares")
  }
)
...
setMethod(
  ...
  definition = function(object, preMerger = TRUE, market = FALSE, ...) {
    ...
    if (preMerger) {
      if (market) {
        return(sum(object@quantityPre, na.rm = TRUE))
      }
      ...
      if (market) {
        return(sum(object@quantityPost, na.rm = TRUE))
      }
    ...
    FOC <- function(quantCand) {
      ...
      if (preMerger) {
        object@quantityPre <- thisQuant
      } else {
        object@quantityPost <- thisQuant
      }
    ...
    if (minResult$convergence != 0) {
      warning("'calcQuantities' nonlinear solver may not have successfully converged. 'BBsolve' reports: '", minResult$message, "'")
    }
...
setMethod(
  ...
  definition = function(object, preMerger = TRUE, revenue = TRUE) {
    ...
    if (!revenue) {
      shares <- (shares / prices) / sum(shares / prices)
    }
...
setMethod(
  ...
  definition = function(object, preMerger = TRUE, revenue = FALSE) {
    ...
    if (!revenue) {
      shares <- (shares / prices) / sum((1 - sum(shares, na.rm = TRUE)) / priceOutside, shares / prices, na.rm = TRUE)
    }
...
setMethod(
  ...
  definition = function(object, preMerger = TRUE, revenue = FALSE) {
    ...
    ## transform revenue shares to quantity shares
    if (!revenue) {
      shares <- (shares / prices) / sum((1 - sum(shares, na.rm = TRUE)) / object@priceOutside, shares / prices, na.rm = TRUE)
    }
