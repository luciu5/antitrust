...
setClass(
  ...
  validity = function(object) {
    ...
    if (!(length(object@normIndex) == 1 &&
      ...
      stop(
        "'normIndex' must take on a value between 1 and ", nprods,
        " or NA"
      )
    }
...
#' @export
setClass(
  Class = "LogitCournotALM",
  contains = "LogitCournot",
  representation = representation(
    parmsStart = "numeric"
  ),
  prototype = prototype(
...
setClass(
  ...
  validity = function(object) {
    margins <- object@margins
    if (any(margins > 1, na.rm = TRUE)) stop("'margins' values must be between 0 and 1")
    if (!is.na(object@mktElast)) {
