...
setClass(
  Class   = "Auction2ndAuction2ndLogitNests",
    ...
    minD <- function(theta){
      ...
      thisMargin <- apply(sharesCond, 1, 
                      function(x){
                        ...
                        isZero <- x == 0
                        x <- (1 - x) ^sigma
                        res <- ifelse(any(isZero), sum(sharesNest[isZero]), 0) 
                        res <- res +  x * sharesNests
                        res <- log(res)/alpha
                        return(res)
                      }
      )
...
logit.nests <- function(prices,shares,margins,
  ...
  result <- new("Auction2ndLogitNests",prices=prices, margins=margins,
