...
setMethod(
  ...
  definition = function(object) {
    ...
    uPost <- meanval + (object@pricePost - object@priceOutside) * alpha
    uPost[is.na(uPost)] <- -Inf
    VPost <- sum(exp(uPost)[subset]) + outVal
...
setMethod(
  ...
  definition = function(object) {
    ...
    if (any(is.na(object@prices))) {
      stop("Compensating Variation cannot be calculated without supplying values to 'prices'")
    }
