...
#'
#' @details Strategy:
#' 1. Calculate profits at collusive prices (pi_PL)
#' 2. Calculate Bertrand punishment profits (pi_B)
#' 3. Calculate optimal deviation profits for each firm (pi_D)
...
setMethod(
  ...
  definition = function(object, preMerger = TRUE, pricesColl, coalition){
    ...
    ## IC: pi_PL - pi_D + delta/(1-delta) * (pi_PL - pi_B) >= 0
    ...
    if(supermarkup_observed <= tol){
      ...
      # All ICs satisfied at delta=1 (static constraint holds)
      ...
      # delta_f = (pi_D_f - pi_PL_f) / (pi_D_f - pi_B_f)
...
setMethod(
  ...
  definition = function(object){
    ...
    } else if(length(object@timingParam) == 0 ||
              all(is.na(object@timingParam))){
      ## Without identified timing parameters, the supplied data identify no
      ## additional post-merger leadership markup.  Keep the post-merger
      ## calculation on the coordinated equilibrium path.
      object@supermarkupPost <- 0
    } else {
...
#' IC_f: pi_PL_f(m) - pi_D_f(m) + delta_f/(1-delta_f) * [pi_PL_f(m) - pi_B_f] >= 0
#' @aliases calcSupermarkup,PriceLeadership-method
...
setMethod(
  ...
  definition = function(object, preMerger = TRUE, isMax = FALSE, subset,
    ...
    if(isMax && length(setdiff(seq_len(nprods),
                               if(preMerger) {
                                 if(is.matrix(object@coalitionPre)) {
                                   which(rowSums(object@coalitionPre) > 1)
                                 } else {
                                   object@coalitionPre
                                 }
                               } else {
                                 if(is.matrix(object@coalitionPost)) {
                                   which(rowSums(object@coalitionPost) > 1)
                                 } else {
                                   object@coalitionPost
                                 }
                               })) > 0){
...
#' g_f(m) = [pi_PL_f - pi_D_f] + delta/(1-delta) * [pi_PL_f - pi_B_f]
#'
#' where:
#' \itemize{
#'   \item pi_PL_f: Profit under price leadership
#'   \item pi_D_f: Profit from optimal deviation
#'   \item pi_B_f: Profit under Bertrand punishment
#'   \item delta: Timing/discount parameter
#' }
#'
#' @export
calcSlack <- function(object, preMerger = TRUE, ...){
  ...
  ## Step 1: Calculate price leadership profits (pi_PL) at current prices
  ...
  ## Step 2: Calculate Bertrand profits (pi_B) - punishment
  ...
  ## Step 3: Calculate deviation profits (pi_D) for each coalition firm
  ...
  if(length(timingParam) == 0 || all(is.na(timingParam))){
    ...
    warning("Timing parameter not specified. Slack values show only immediate payoff difference [pi_PL - pi_D].")
  } else {
