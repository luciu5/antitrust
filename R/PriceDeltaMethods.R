...
setGeneric(
  name = "calcPriceDelta",
  def = function(object, levels = FALSE, market = FALSE, party = FALSE, isMax = FALSE, index = c("paasche", "laspeyres"), ...) {
    standardGeneric("calcPriceDelta")
  }, signature = "object"
)
...
setMethod(
  ...
  definition = function(object, levels = FALSE, market = FALSE, party = FALSE, isMax = FALSE, index = c("paasche", "laspeyres"), subset, ...) {
    ...
    if (market || party) {
      ...
      if (party) {
        if (index == "paasche") {
          shares <- calcShares(object, preMerger = FALSE)
        } else {
          shares <- calcShares(object, preMerger = TRUE)
        }
    ...
    minResult <- nleqslv::nleqslv(object@priceStart, FOC,
      method = "Broyden",
      control = list(
    ...
    if (isMax) {
      ...
      if (any(eigen(hess)$values > 0)) {
        warning("Hessian of first-order conditions is not positive definite. Price vector may not maximize profits. Consider rerunning 'calcPrices' using different starting values")
      }
    }
    ...
    if (levels) {
      deltaPrice <- calcPrices(object, FALSE) - calcPrices(object, TRUE)
    }
